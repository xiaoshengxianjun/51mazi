...
const customElectronAPI = {
  ...
  // --------- Minimax AI 相关 ---------
  // 设置 API Key
  setMinimaxApiKey: (apiKey) => ipcRenderer.invoke('minimax:set-api-key', apiKey),
  // 获取 API Key
  getMinimaxApiKey: () => ipcRenderer.invoke('minimax:get-api-key'),
  // 设置 URL
  setMinimaxUrl: (url) => ipcRenderer.invoke('minimax:set-url', url),
  // 获取 URL
  getMinimaxUrl: () => ipcRenderer.invoke('minimax:get-url'),
  // 验证 API Key
  validateMinimaxApiKey: () => ipcRenderer.invoke('minimax:validate-api-key'),
  // 拆书分析
  analyzeBookWithMinimax: (options) => ipcRenderer.invoke('minimax:analyze-book', options),
  // 监听拆书进度
  onMinimaxAnalyzeProgress: (callback) => {
    const handler = (_, progressData) => callback(progressData)
    ipcRenderer.on('minimax:analyze-progress', handler)
    return () => ipcRenderer.removeListener('minimax:analyze-progress', handler)
  },
  // 监听拆书结果片段
  onMinimaxAnalyzeChunkResult: (callback) => {
    const handler = (_, chunkResult) => callback(chunkResult)
    ipcRenderer.on('minimax:analyze-chunk-result', handler)
    return () => ipcRenderer.removeListener('minimax:analyze-chunk-result', handler)
  },
  // 保存拆书分析
  saveBookAnalysis: (options) => ipcRenderer.invoke('save-book-analysis', options),
  // 获取书籍保存目录
  getBooksDir: () => ipcRenderer.invoke('get-books-dir'),
  // 取消拆书分析
  cancelAnalysis: () => ipcRenderer.invoke('minimax:cancel-analysis'),
  // 打开目录选择对话框
  openDirectoryDialog: () => ipcRenderer.invoke('open-directory-dialog'),
  // 打开小说下载器
  openNovelDownloader: (downloadPath) => ipcRenderer.invoke('open-novel-downloader', downloadPath),
  // 停止监控下载文件夹
  stopDownloadWatcher: () => ipcRenderer.invoke('stop-download-watcher'),
  // 监听下载完成事件
  ...
  // 解析 EPUB 文件
  parseEpub: (epubBuffer) => ipcRenderer.invoke('parse-epub', epubBuffer)
}
