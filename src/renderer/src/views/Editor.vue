      ...
      <div class="panel-section">
        ...
        <div v-show="chaptersExpanded" class="section-content">
          ...
          >
            <template #default="{ node }">
              <div class="custom-tree-node">
                <span v-if="!editingNode || editingNode.path !== node.data.path">{{
                  node.label
                }}</span>
                <el-input
                  v-else
...
async function handleNoteClick(data, node) {
  if (data.type === 'note') {
    ...
    const res = await window.electron.readNote(bookName, parent.name, data.name)
    if (res.success) {
      currentFile.value = {
        name: data.name,
        type: 'note',
...
async function saveContent() {
  ...
  if (currentFile.value.type === 'note') {
    const res = await window.electron.editNote(
      bookName,
      currentFile.value.notebook,
...
// 组件挂载时加载书籍数据
onMounted(async () => {
  sortOrder.value = await window.electron.getSortOrder(bookName)
  await loadChapters()
  // 加载笔记目录
  notesTree.value = await window.electron.loadNotes(bookName)
...
async function confirmEdit(node) {
  ...
  try {
    const result = await window.electron.editNode(bookName, payload)
    if (result.success) {
...
async function deleteNode(node) {
  ...
  try {
    ...
    const result = await window.electron.deleteNode(bookName, payload)
    if (result.success) {
...
async function sortVolumes() {
  ...
  await window.electron.setSortOrder(bookName, sortOrder.value)
  chaptersTree.value = [...chaptersTree.value].reverse()
}

// 创建笔记本
async function createNotebook() {
  const result = await window.electron.createNotebook(bookName)
  if (result.success) {
...
async function createNote(node) {
  ...
  const result = await window.electron.createNote(bookName, notebookName)
  if (result.success) {
...
async function confirmEditNote(node) {
  ...
  if (editingNoteNode.value.type === 'folder') {
    ...
    result = await window.electron.renameNotebook(bookName, editingNoteNode.value.name, newName)
...
async function deleteNoteNode(node) {
  ...
  try {
    ...
    if (node.data.type === 'folder') {
      result = await window.electron.deleteNotebook(bookName, node.data.name)
    } else if (node.data.type === 'note') {
      result = await window.electron.deleteNote(bookName, node.parent.data.name, node.data.name)
    }
    if (result && result.success) {
      ElMessage.success('删除成功')
      notesTree.value = await window.electron.loadNotes(bookName)
