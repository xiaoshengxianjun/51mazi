...
![AI scene image example](https://raw.githubusercontent.com/xiaoshengxianjun/51mazi/main/static/ai_scene.png)
...
- **Art style / shot / environment / lighting**: Similar style presets as character images, plus scene-oriented options
...
<span id="ai-comic"></span>
### 📖 AI Comic

On the **chapter editor** page, click **AI Comic** in the **top-right** of the editing area (below **AI Scene Image**). Two content sources are supported:

- **Selection mode**: selected text (~**100–1000** effective characters)
- **Whole chapter mode**: full chapter when nothing is selected (~**200–5000** effective characters)

The drawer lets you pick comic style (manga / manhwa / American comic / Chinese comic), panel count (4 / 6 / 8), frame size, and image provider. **Generate storyboard** (DeepSeek API key) splits the text into editable panel descriptions; **Generate comic panels** renders each panel sequentially. Output is saved under **`comic_panels/{chapter_title_timestamp}/`** with `storyboard.json` and `panel_XX.png` files. Per-panel retry is supported. Results are reference panels; character appearance may vary across panels.
...
| AI Assistance | ✅ DeepSeek naming/polish/continuation + Tongyi Wanxiang cover/character/scene image | ❌ Lack of AI Features |
...
14. In Character Profile, complete characters: fill in appearance and biography, and add multiple character images (different styles or poses) for the same character
...
- **AI Feature Expansion**: Keep iterating on creative assistance beyond current capabilities
  ...
  - ✅ **AI Character Image (Tongyi Wanxiang)**: Vertical full-body images, multiple styles and poses, multiple images per character
...
- ✅ **AI-Assisted Creation**: DeepSeek naming/polish/continuation + Tongyi Wanxiang cover/character/scene image, improving creative efficiency
...
- **Keywords**: Desktop Application, Rich Text Editing, Canvas Drawing, Relationship Graph, Novel Writing, Dictionary, Forbidden Word Detection, Character Highlighting, AI Assistance, AI Cover, AI Character Image, AI Scene Image, Text-to-Image, AI Writing, Intelligent Naming, Character Roster, Novel Download, Multi-source
...
<span id="tags"></span>
## 🏷️ Tags
`#Electron` `#Vue3` `#DesktopApplication` `#RichTextEditing` `#CanvasDrawing` `#RelationshipGraph` `#NovelWriting` `#CharacterRoster` `#FrontendDevelopment` `#Dictionary` `#ForbiddenWordDetection` `#CharacterHighlighting` `#AIAssistance` `#AICovers` `#AICharacterImage` `#AISceneImage` `#TextToImage` `#AIWriting` `#TongyiWanxiang` `#DeepSeek` `#IntelligentNaming` `#NovelDownload`
