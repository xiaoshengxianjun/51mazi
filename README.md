...
<span id="ai-生成人物图"></span>
### 🎨 AI 生成人物图

![AI生成人物图示例](https://raw.githubusercontent.com/xiaoshengxianjun/51mazi/main/static/ai_character.png)

在「人物档案」编辑抽屉中点击 **AI 生成人物图**，可选择画风（如日系动画、吉卜力、写实摄影等）、填写形象描述与构图姿态，生成多张竖版全身人物图；选择一张**确认使用**即可加入该人物的人物图列表，支持为同一角色保存多张不同风格或姿态的图片。

<span id="ai-场景图"></span>
### 🖼️ AI 场景图

![AI 场景图示例](https://raw.githubusercontent.com/xiaoshengxianjun/51mazi/main/static/ai_scene.png)

在**章节编辑页**选中一段正文描写，点击编辑区右上角的 **AI 场景图**（位于「AI 续写」下方）。选区**有效字数**需在约 **100～1000 字**范围内（与软件字数统计规则一致，不含空白）；通过后打开右侧抽屉，可配置：

- **输出尺寸**：横版 1280×720（默认）或方图 1280×1280  
- **画风 / 景别 / 环境 / 光线**：与人物图类似的风格选项，并增加场景镜头维度  
- **画面描述**：打开时按节选智能预填，可改为更偏「画面向」的描述（最多 500 字）  
- **AI 提炼画面（DeepSeek，可选）**：将整段节选压缩为适合文生图的短描述，需配置 DeepSeek API Key  
- **反向提示词**：可选，减少模糊、畸形、水印等  

图片由**通义万相**生成，成功后**直接保存**到本书目录下的 **`scene_images`** 文件夹（`scene_时间戳.png`），抽屉内可预览大图并**复制完整本地路径**；同一会话可多次「再生成一张」。**通义万相 API Key 为必需**（与 AI 封面、人物图共用设置项）。

<span id="ai-漫画"></span>
### 📖 AI 漫画

在**章节编辑页**点击编辑区右上角的 **AI 漫画**（位于「AI 场景图」下方）。支持两种内容来源：

- **选段模式**：选中正文（有效字约 **100～1000 字**）  
- **整章模式**：无选区时使用当前章节全文（有效字约 **200～5000 字**）

打开右侧抽屉后可配置漫画风格（日漫 / 韩漫 / 美漫 / 国漫）、分镜格数（4 / 6 / 8）、画幅与图像服务。点击 **生成分镜脚本**（需 DeepSeek API Key）将正文拆分为可编辑的分镜描述，再 **生成漫画分镜图** 逐格出图。结果保存在本书目录 **`comic_panels/{章节名_时间戳}/`**，含 `storyboard.json` 与各格 `panel_XX.png`；支持单格重试。结果为分镜参考图，跨格角色外貌可能不一致。
...
- **AI 润色**: 一键润色选中文本或整章内容，优化表达与可读性，同时保持原意与风格
- **AI 续写**: 支持输入续写要求（可选），AI 生成续写结果后可复制或确认插入到章节末尾
  - 章节字数达到目标字数时将提示新建章节
  - 续写字数按章节目标字数的 120% 上限动态计算（上限总字数 - 当前章节字数）
  - 当前章节为空时优先承接上一章节末尾内容续写；若无上一章则需当前章至少 200 字才允许续写
- **AI 场景图**: 选中一段正文（有效字数约 100～1000 字），在抽屉中配置画风与画面描述后由通义万相生成场景插图，保存到本书 `scene_images`，可选 DeepSeek 提炼画面描述（详见上文 [AI 场景图](#ai-场景图)）
...
- **双视图模式**：支持**卡片模式**与**表格模式**切换，按书籍记忆上次选择
...
- **AI 生成人物图**：集成通义万相，一键生成竖版全身人物图（720×1280）
  - 支持**画风选择**：日系动画、吉卜力风格、复古日漫、写实摄影、3D 渲染、欧美卡通、国风插画、水彩、水墨、厚涂、赛博朋克、像素艺术等
  - 支持**构图与姿态**：站姿全身、半身特写、动态姿势等
...
![下载小说](https://raw.githubusercontent.com/xiaoshengxianjun/51mazi/main/static/novelDownload.png)

*下载小说页面 - 多书源搜索、选中书籍后底部悬浮操作区支持下载并加入书架或导出 TXT*

下载小说功能支持按书名或作者搜索、选择书源，将网络小说下载到本地并加入书架或导出为 TXT（仅供个人学习研究，请遵守当地法律法规）。
...
- **AI 辅助创作**：集成 DeepSeek + 通义万相
  ...
  - **AI 润色 / AI 续写（DeepSeek）**：编辑器内润色选区或整章；续写可带要求、受目标字数 120% 上限约束，结果可插入章末
  - **AI 小说封面（通义万相）**：按书名/笔名/尺寸/提示词生成多张封面，选择后一键设为书籍封面
  - **AI 人物图（通义万相）**：在人物档案中按画风与形象描述生成竖版全身人物图，支持多张保存到人物图列表，便于不同风格/姿态展示
  - **AI 场景图（通义万相 + 可选 DeepSeek）**：章节中选中文本生成场景插图，落盘至 `scene_images`，可选 DeepSeek 将节选提炼为画面描述
- 下载小说：多书源搜索、下载并加入书架或导出 TXT，底部悬浮操作卡与进度展示
...
| AI 辅助 | ✅ DeepSeek 起名/润色/续写 + 通义万相 封面/人物图/场景图 | ❌ 缺乏 AI 功能 |
| 人物图管理 | ✅ 多张人物图列表，AI 生成竖版全身 | ❌ 单一头像或需外链 |
| 智能操作 | ✅ 智能拖拽，快捷键支持 | ❌ 操作繁琐 |
| 多主题 | ✅ 多种主题 | ❌ 单一主题 |
| 用户指南 | ✅ 内置完整指南 | ❌ 需要外部文档 |
| 下载小说 | ✅ 多书源搜索、加入书架/导出 TXT | ❌ 需手动复制或外部工具 |
| 跨平台 | ✅ 全平台支持 | ❌ 平台限制 |
| 免费使用 | ✅ 完全免费 | ❌ 付费订阅 |
...
8. （可选）在人物档案中为角色添加**人物图**：选择本地图片或点击 **AI 生成人物图**，选择画风与填写形象描述后生成竖版全身图，可多张保存到该人物的人物图列表
9. （可选）在章节编辑页选中一段正文（约 100～1000 有效字），点击 **AI 场景图**，在抽屉中调整画风与画面描述后生成场景插图（需通义万相 Key；可选 DeepSeek 提炼画面）
10. 尝试使用地图设计工具
   - 使用画笔工具绘制地形轮廓
   - 使用油漆桶填充区域颜色
   - 拖拽资源图标添加建筑和地标
11. 建立人物关系图谱（可设置头像）
12. 创建词条字典，管理故事中的专有名词
13. 体验 AI 随机起名功能
   - 在设置中配置 DeepSeek API Key（可选）
   - 使用 AI 智能起名为角色生成名字
   - 尝试不同参数设置，体验 AI 的智能理解能力
14. 在人物档案中完善角色：填写形象介绍与生平，并可为同一角色添加多张人物图（不同风格或姿态）
15. （可选）使用下载小说：左侧菜单进入「下载小说」，选择书源、输入书名或作者搜索，选中书籍后可「下载并加入书架」或「导出为 TXT」，操作区悬浮在页面底部便于查看进度
...
5. 在章节中针对关键段落使用 **AI 场景图**：多试画风与「AI 提炼画面」，将生成图纳入 `scene_images` 做章前参考或素材归档
6. 使用下载小说功能：多书源搜索后下载到本地或导出 TXT，用于阅读参考与个人备份（请遵守版权与当地法规）
7. 结合地图设计构建世界观
   - 使用形状工具绘制精确的地形边界
   - 使用文字工具添加地名标注
   - 利用资源图标快速构建地图元素
...
8. 利用随机名字生成器丰富角色
   ...
   - 利用批量生成功能，快速筛选合适的名字
9. 为书架设置密码保护，确保所有数据安全
10. 使用关系图谱的头像功能增强视觉效果
11. 建立完整的词条字典体系，分类管理故事中的专有名词、地名、组织等
12. 在编辑器中开启禁词提示，实时检测并标记敏感词汇
13. 使用人物高亮功能，快速定位角色在文本中的出现位置
14. 查看内置用户指南，了解更多高级功能和技巧
...
- **AI 功能扩展**: 在现有能力基础上持续迭代更多创作辅助
  - ✅ **AI 随机起名（DeepSeek）**：多种类型与参数，失败可降级本地生成
  - ✅ **AI 润色 / AI 续写（DeepSeek）**：选区或整章润色；续写可带要求并插入章末
  - ✅ **AI 小说封面（通义万相）**：按书名/笔名/风格与尺寸生成封面
  - ✅ **AI 人物图（通义万相）**：竖版全身人物图，多画风与构图，多张入库
  - ✅ **AI 场景图（通义万相 + 可选 DeepSeek）**：选中文本生成场景插图，落盘 `scene_images`，可选提炼画面描述
  - 🔮 **AI 摘要**：自动生成章节摘要
  - 🔮 **AI 对话生成**：生成角色对话
...
- ✅ 跨平台支持，使用便捷
- ✅ 功能全面，满足各种创作需求
- ✅ **AI 辅助创作**：DeepSeek 起名/润色/续写 + 通义万相 封面/人物图/场景图，提升创作效率
- ✅ 智能编辑器功能（人物高亮、禁词提示、段落拖拽）
- ✅ 专业地图设计工具，内置资源库
- ✅ 词条字典管理，树形结构灵活组织
- ✅ 智能操作体验，提升创作效率
- ✅ 下载小说：多书源搜索、下载并加入书架或导出 TXT，方便阅读与备份
- ✅ 界面美观，用户体验优秀
- ✅ 内置用户指南，快速上手
- ✅ 开源免费，持续更新
...
### 📚 相关链接
- **官网**: [www.51mazi.com](https://www.51mazi.com)
- **项目地址**: [GitHub - 51mazi](https://github.com/xiaoshengxianjun/51mazi)
- **技术栈**: Electron + Vue 3 + TipTap + Element Plus + Pinia + DeepSeek + 通义万相
- **关键词**: 桌面应用、富文本编辑、Canvas 绘图、关系图谱、小说写作、词条字典、禁词检测、人物高亮、AI辅助、AI封面、AI人物图、AI场景图、文生图、AI写作、智能起名、人物谱、下载小说、多书源
...
<span id="标签"></span>
## 🏷️ 标签
`#Electron` `#Vue3` `#桌面应用` `#富文本编辑` `#Canvas绘图` `#关系图谱` `#小说写作` `#人物谱` `#前端开发` `#词条字典` `#禁词检测` `#人物高亮` `#AI辅助` `#AI写作` `#通义万相` `#DeepSeek` `#智能起名` `#AI场景图` `#文生图` `#下载小说`
